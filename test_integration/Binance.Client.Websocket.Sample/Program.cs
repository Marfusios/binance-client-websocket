...
class Program
{
    ...
    static async Task Main(string[] args)
    {
        ...
        using (var futuresApiClient = new WebsocketClient(fUrl))
        {
            apiClient.Name = "Binance-1";
            apiClient.ReconnectTimeout = TimeSpan.FromMinutes(10);
            apiClient.ReconnectionHappened.Subscribe(type =>
                Log.Information($"Reconnection happened, type: {type}"));

            futuresApiClient.Name = "Binance-f";
            futuresApiClient.ReconnectTimeout = TimeSpan.FromMinutes(10);
            futuresApiClient.ReconnectionHappened.Subscribe(type =>
                Log.Information($"Reconnection happened, type: {type}"));

            using var client = new BinanceWebsocketClient(NullLogger.Instance, apiClient,
            ...
            using var fClient = new BinanceWebsocketClient(NullLogger.Instance, futuresApiClient, new FundingSubscription("btcusdt"));

            SubscribeToStreams(client, apiClient);
            SubscribeToStreams(fClient, apiClient);

            apiClient.Start().Wait();
            futuresApiClient.Start().Wait();
    ...
    static void SubscribeToStreams(BinanceWebsocketClient client, IWebsocketClient connection)
    {
